...
def game(request):
    ...
    return render(request, 'accounts/game.html', {'ongoing_rooms': ongoing_rooms})
...
def create_room(request):
    if request.method == "POST":
        ...
        if int(participant_count) < 3:
            ...
            return render(request, 'accounts/create_room.html')

        # Room 모델에 저장
        Room.objects.create(
            name=room_type,
            mission=", ".join(mission),  # 미션을 쉼표로 구분해 저장
            reveal_date=reveal_date,
            participant_count=int(participant_count),
            is_active=True
        )
        messages.success(request, "마니또 방이 성공적으로 생성되었습니다.")
        return redirect('accounts:game')
